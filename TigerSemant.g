...
expr returns [Type t]
  ...
  | #( BINOP a=expr b=expr
	 ...
	 String op = #expr.getText();
		...
		{semantError(#expr, "The arguments of a binary operator must be of the same type.");}
	    
	    if(a instanceof VOID)
		{semantError(#expr, "Binary operators cannot be applied to void types.");}
  ...
  | #( CALL z:ID 
	...
	*/
	...
	if(jello == null)
	    {semantError(z, "Error, no function found with the given signature, too many arguments.");}
    
	if(a != jello.fieldType)
	    {semantError(z, "Error, no function found with the given signature, mismatched arguments.");}
	...
	if(jello != null)
	    {semantError(z, "Error, no function found with the given signature, too few arguments.");}
  ...
  | #( RECORD fuckoff:ID (#(FIELD ID a=expr))*
